...
class Template extends Core
	{
		 ...
		 */
		public function __construct($config_file = "./config.template.php")
		{
		...
		public function output($name, $type = "html")
		{
			if (! isset($name))
				{
					Core::set_error("Bitte geben Sie ein Template zur Ausgabe an!");
				}
			if (! isset(self::$template[$name])) self::load($name);
			
			$timers = "";
			foreach(parent::$_Tmr as $timer)
			{
				$timers .= "$timer<br/>";
			}
			
			$errors = "";
			foreach(parent::$_Err as $error)
			{
				$errors .= "$error<br/>";
			}
			
			self::assign($name, "<!--TIMERS-->", $timers);
			self::assign($name, "<!--ERRORS-->", $errors);
