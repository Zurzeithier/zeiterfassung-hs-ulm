 ...
 */
 class MsSql implements iSql
 {
  ...
  public function __construct( $config_file = "./configs/mssql.conf.php5" )
  {
  ...
  {
   if ( ! self::connect() ) trigger_error( 'Konnte keine Verbindung zum Datenbank-Server (' . self::$hostname . ') hergestellt! ' . self::$mssql_error );
   if ( ! self::select_db() ) trigger_error( 'Konnte keinen Zugriff auf die Datenbank (' . self::$database . ') herstellen! ' . self::$mssql_error );
  }
