...
class Session extends Core
	{
		...
		protected static $timeout_secs  = 300;
		...
		public function login()
		{
			...
			$found = parent::ObjSQL()->query_first($query);
			if (! isset($found["MId"]))
				{
					parent::set_error("access denied for ".$_SERVER["REMOTE_ADDR"]."!");
					return false;
				}
			$_SESSION["SessionTimer"] = time();
			$_SESSION["UserID"]       = $found["MId"];
			$_SESSION["UserNAME"]     = $found["Vornamen"]." ".$found["Namen"];
			$_SESSION["Vornamen"]     = $found["Vornamen"];
			$_SESSION["LoginNamen"]   = $found["LoginNamen"];
			$_SESSION["Namen"]        = $found["Namen"];
			$_SESSION["GroupID"]      = 2;
		...
		{
			$away = (time() - $_SESSION["SessionTimer"]);
			return (($_SERVER["REMOTE_ADDR"] == $_SESSION["ClientIP"]) && ($away < self::$timeout_secs));
		}
